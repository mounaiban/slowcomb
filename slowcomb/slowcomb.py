...
class PBTreeCombinatorics(Combinatorics):
    ...
    * Level 3 comprises nodes 10 thru 21
    ...
    def _get_seq_addresses(self, ii):
        ...
        The exact method of resolving the address to a term of the
        ...
        The address is presented as a tuple of ``int`` indices, each
        ``int`` corresponding to the index of an item on the source
        sequence, _seq_src, and also the sub nodes to navigate into.
        These addresses begin at zero. The algorithm used in this
        method gets elements, from the furthest to the closest to the
        root.

        Deriving the First Element of the Address
        =========================================
        ...
        This actually the last element of the address, 0. Recall that
...
class CatCombination(PBTreeCombinatorics):
    ...
    def _get_comb(self, ii):
        ...
        ----------------------
        Each element in the addresses tuple returned by _get_seq_addresses
        ...
        Now, get the combinatorics tree address with the resolved
        internal index:

        >>> comb._get_seq_addresses(ii)
        (0, 2, 0, 0, 2)
        ...
        Well done! You have successfully resolved a CatCombinator tree
        address.


        """
        addrs = self._get_seq_addresses(ii)
        out = []
...
class Permutation(PBTreeCombinatorics):
    ...
    def _get_perm(self, ii):
        ...
        Treatment of Addresses
        ----------------------
        The address returned by _get_seq_addresses() is a tuple which is
        regarded as a navigation route through the permutation tree.
        Each element of the address is coincidentially a reference to
        ...
        _get_seq_addresses will be (0, 2, 2, 0, 0). Each number in
...
class PermutationWithRepeats(Permutation):
    ...
    * This permutator was almost called PokiesPermutator, due to the
    ...
    def _get_perm(self, ii):
        ...
        Treatment of Addresses
        ----------------------
        The address tuple returned by _get_seq_addresses() is used
        ...
        ``addrs``, and returned by _get_seq_addresses() should be 
        (0, 1, 0, 2).
